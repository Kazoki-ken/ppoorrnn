...
class Videos(object):
    ...
    def _scrapVideosInfo(self, div_el):
        ...
        for img_tag in div_el.find_all("img", src=True):
            try:
                # Updated to check different attributes as data-mediumthumb seems deprecated or changed
                # Using 'src' or 'data-image' or 'data-thumb_url'
                url = img_tag.attrs.get("data-image") or img_tag.attrs.get("data-thumb_url") or img_tag.attrs.get("src")
                if url and isVideoPhoto(url):
                    data["img_url"] = url
                    break
            except Exception as e:
                pass
        ...
        for var_tag in div_el.find_all("var", { "class" : "duration" } ):
            try:
                # Direct text extraction is safer
                data["duration"] = var_tag.get_text(strip=True)
                break
            except Exception as e:
                pass

        # scrap rating
        # Rating seems to be removed from the list view in some versions or tests.
        # Making it optional to allow the item to be returned even if rating is missing.
        ...
        # If rating is None, we can still return data if other fields are present.
